...
import { settings, songSections, songs, videos } from "./schema";
...
export async function seedDatabase(db: Db) {
  ...
  const { sections, tutorials, ...song } = DONT_THINK_TWICE;
  ...
  await db.insert(videos).values(tutorials.map((video) => ({ songId: created.id, ...video })));
}
